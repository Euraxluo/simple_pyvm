...
msuccess=0 #
merror=0 #
for i in `ls test_*.pyc`;do  mtotal=`echo $((mtotal+=1))` ;t=`/tmp/tmp.Q2zFhEdgvl/cmake-build-debug/simple_pyvm  $i |grep SUCCESS!!! |wc -l` ;if  [[ $t == 0 ]] ;then merror=`echo $((merror+=1))`;echo -e "\033[41m ${i}\033[0m";else msuccess=`echo $((msuccess+=1))`;fi ;done #
